...
%   Developed by Pablo Vicente-Munuera
    ...
    for imK = 1:size(lee_matrices,1)
        ...
        if (lee_matrices(imK).isdir == 0 && (size(strfind(lower(lee_matrices(imK).name), lower('ItFinal')), 1) == 1 || isempty(strfind(lower(lee_matrices(imK).name), lower('It'))) == 1))% && size(strfind(lower(lee_matrices(imK).name), lower('CELS')), 1) == 0)

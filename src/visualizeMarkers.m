function [] = visualizeMarkers(distanceMatrix, nameFiles, markersNames, markersWeWantToShow, algorithm, algorithmWeWantToShow, typeOfGCD, kindsInestability, NBiopsia, estability)
...
%     fclose(fid);
    ...
    [Y,stress,disparities] = mdscale(newMatrix, 3, 'Start', 'random');
    colorsVectors = zeros(size(newMatrix, 1), 3);
    
    numClasses = size(kindsInestability, 1)+size(algorithmWeWantToShow,2 + 1);
    colors = hsv(size(kindsInestability, 1));
    shapes = {'<','x','h','*','>','p','.','+','s','d','v','^'};
    ...
    for i=1:size(points, 1)
        ...
        if shapePoint ~= -1
            for actualBiopsia = 1:size(NBiopsia, 1)
                nameBiopsia = strsplit(NBiopsia{actualBiopsia}, '-');
    ...
    %h = round(h);
    for actualEstabilidad = 1:size(kindsInestability, 1)
       h(actualEstabilidad, :) = plot(0,0, 'o', 'color', colors(actualEstabilidad, :), 'MarkerFaceColor', colors(actualEstabilidad, :));
    end
    for numAlgorithm = 1:size(algorithmWeWantToShow,2)
        h(size(kindsInestability, 1) + numAlgorithm, :) = plot(0,0, shapes{numAlgorithm}, 'color', 'black');
    end
    hold off;
